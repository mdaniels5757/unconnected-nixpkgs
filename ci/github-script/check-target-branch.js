...
async function checkTargetBranch({ github, context, core, dry }) {
  ...
  const rebuildsAllTests = changed.labels['10.rebuild-nixos-tests'] ?? false
  core.info(`checkTargetBranch: PR causes ${maxRebuildCount} rebuilds and ${rebuildsAllTests ? 'does' : 'does not'} rebuild all NixOS tests.`)
